...
- `elevationProfile`: 4-8 points along the route from trailhead (mile 0, elevationFt as the trailhead's approximate base elevation gain of 0) to the end, monotonically increasing `mile`. This is a **stylized approximation** for a drag-to-explore elevation chart, not GPS/survey data — round numbers are fine. Add a `label` only at genuinely notable points (summit, major landmark, junction, water crossing) — most points don't need one.
- Total elevation gain implied by the profile's peak should roughly match the trail's existing `elevationGainFt` in `trails.ts`; if your research finds the existing figure is wrong, flag it in your final report rather than silently overriding it.
